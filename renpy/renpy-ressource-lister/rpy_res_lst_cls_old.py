...
__title__ = 'Ren`py Ressource Lister'
__license__ = 'MIT'
__author__ = 'madeddy'
...
class RRL(object):
    ...
    def valid_path(self):
        ...
        real_path = os.path.realpath(self.chkdir)
        if not os.path.isdir(real_path) and os.path.islink(real_path):
    ...
    def init_register_file(self):
        ...
        if os.path.isfile(self.outfile) and not os.path.islink(self.outfile):
            timestamp = strftime('%d%b%Y_%H:%M:%S', localtime())
            os.rename(self.outfile, os.path.splitext(
                self.outfile)[0] + '.' + timestamp + '.bup')
    ...
    def format_test(self, testobj):
        ...
        supp_state = None
        if os.path.isfile(testobj) and not os.path.islink(testobj):
            mime_type = magic.from_file(testobj, mime=True)

            # print('get mime_type', mime_type)

            if self.typus not in mime_type:
                return supp_state
    ...
    def dir_lister(self):
        ...
        for path, dirs, files in os.walk(self.chkdir):
            ...
            self.tmp_lst.append(f'# ### Current directory: {path}{os.path.sep}{subdirs if dirs else ""}')
            ...
            for fn in files:
                ...
                if format_status == 'supported':
                    ...
                    rel_path = os.path.relpath(fullpath, self.chkdir)
                    self.tmp_lst.append(
...
def parse_args():
    ...
    def valid_switch():
        ...
        if not args.def_img and not args.def_aud and not args.def_vid:
            parser.print_help()
            raise parser.error('No task requested; either -i, -a or -v is required.')
